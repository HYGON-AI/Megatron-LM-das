...
from megatron.core.pipeline_parallel.pipeline_parallel_layer_layout import PipelineParallelLayerLayout
...
class PipelineParallelLayerLayoutDualpipeV:
    ...
    def validate_layer_layout(self, num_layers: int, mtp_num_layers: int):
        ...
        assert LayerType.mtp not in self.layout[0][0], (
    ...
    def get_layer_offset(
        ...
        offset = 0
        for _pp_rank in range(self.pipeline_model_parallel_size if vp_stage == 0 else pp_rank):
    ...
    def get_num_stages_from_str(layout: str):
        ...
        assert len(layout_list) % get_args().pipeline_model_parallel_size == 0, (
            f"The length of pipeline_model_parallel_layout must be equal to"
            f" 2 * pipeline_model_parallel_size"
        )
        assert len(layout_list) // get_args().pipeline_model_parallel_size == 2, (
            f"The length of pipeline_model_parallel_layout must be equal to"
            f" 2 * pipeline_model_parallel_size"
        )
        ...
        return get_args().pipeline_model_parallel_size
