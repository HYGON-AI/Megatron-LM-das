...
# from ...training.tokenizer.tokenizer import get_tokenizer
...
def stack_and_reshape_images(samples, temporal_patch_size, patch_size):
    """
    将 samples 中的 imgs 扁平化并 reshape 成 [num_tiles, 3, temporal_patch_size, patch_size, patch_size]
"""
    all_imgs = []
    for s in samples:
        for img in s.imgs:
            ...
            per_tile_size = 3 * temporal_patch_size * patch_size * patch_size

            if num_elements % per_tile_size != 0:
                raise ValueError(
                    f"img size {num_elements} is not divisible by per_tile_size {per_tile_size}. "
                    f"Check your temporal_patch_size or patch_size."
                )

            num_tiles = num_elements // per_tile_size
            img_tensor = img_tensor.view(num_tiles, 3, temporal_patch_size, patch_size, patch_size)
            all_imgs.append(img_tensor)

    if len(all_imgs) > 0:
        all_imgs = torch.cat(all_imgs, dim=0)
    else:
        all_imgs = torch.empty([0, 3, temporal_patch_size, patch_size, patch_size], dtype=torch.float32)
...
class TaskEncoder(DefaultTaskEncoder[Union[VQASample, ChatMLSample], ImageTaskSample, VQATaskBatch, dict]):
    ...
    def _flatten_visual_inputs(self, visuals, is_image: bool = True):
        ...
        for visual in visuals:
            if is_image:
            # 单帧图片 tile 成 temporal_patch_size 帧
                resized_height, resized_width = visual.shape[-2:]
                # tile 成 temporal_patch_size 帧
                patches = np.tile(np.array(visual), (self.temporal_patch_size, 1, 1, 1))  # shape: (T, C, H, W)
            else:
                # 视频输入
                assert len(visual) % self.temporal_patch_size == 0, \
                    f"Video length {len(visual)} must be divisible by temporal_patch_size {self.temporal_patch_size}"
    ...
    def encode_chatml(self, sample: ChatMLSample):
        # modify get_visual_transform to add more augmentations
    ...
    def batch(self, samples: List[ImageTaskSample]) -> VQATaskBatch:
        imgs = stack_and_reshape_images(samples, temporal_patch_size=self.temporal_patch_size, patch_size=self.patch_size)
        ...
        megatron_batch = self.encode_batch(batch)
        return megatron_batch
    ...
    def encode_batch(self, batch):
        get = lambda x: batch[x] if isinstance(batch, dict) else getattr(batch, x)
    
        return {
            "__key__": get("__key__"),
            "__restore_key__": get("__restore_key__"),
            "imgs": get("imgs"),
            "videos": get("videos"),
            "image_thw_grids": get("image_thw_grids"),
            "video_thw_grids": get("video_thw_grids"),
            "image_input_mask": get("image_input_mask"),
            "video_input_mask": get("video_input_mask"),
            "second_per_grid_ts": get("second_per_grid_ts"),
            "text": get("text"),
            "target": get("target"),
            "tokens": get("text"),
            "labels": get("target"),
            "attention_mask": torch.ones_like(get("text")),
            "loss_mask": torch.ones_like(get("text")),
        }
