        ...
        if self.reuse_fp32_param:
            ...
            if data_parallel_world_size == 1:
                ...
                for buffer in self.buffers:
                    ...
                    shard_res_and_buffer_model_param = torch.zeros(buffer_numel * 2, dtype=torch.bfloat16,
                                                                   device=buffer.param_data.device)
                    shard_main_param_int32_view_buffer = torch.empty(buffer_numel, dtype=torch.int32,
                                                                     device=buffer.param_data.device)
                ...
                for model_fp16_params_this_group, shard_fp32_from_float16_group in zip(
                    ...
                    for i, (model_param, shard_fp32_main_param) in enumerate(
                        ...
                        data_start_index, data_end_index, bucket_id = self.buffers[gbuf_index].param_index_map[
                            model_param]
                        reuse_data_ptr(shard_fp32_from_float16_group[i], self.shard_main_param_res_buffers[gbuf_index],
                                       data_start_index)
                        old_param_data = model_param.data
                        model_param.data = self.shard_main_param_res_buffers[gbuf_index][
                                           data_start_index + data_end_index: 2 * data_end_index].view(
                            old_param_data.shape)
                        model_param.data.detach().copy_(old_param_data)
                        self.shard_fp32_param_fp16_view_group.append(
                            self.shard_main_param_res_buffers[gbuf_index][2 * data_start_index: 2 * data_end_index])
                ...
                for buffer in self.buffers:
                    ...
                    shard_main_param_res_buffer = torch.zeros(res_numel, dtype=torch.bfloat16,
                                                              device=buffer.param_data.device)
                    ...
                    for bucket in buffer.buckets:
                        ...
                        shard_main_param_int32_view_bucket = torch.empty(param_data_dp_numel, dtype=torch.int32,
                        ...
                        self.model_param_bucket_and_res_map[bucket.param_data] = self.shard_main_param_res_buffers[-1][
                                                                                 bucket_res_numel: bucket_res_numel + param_data_dp_numel]
                        self.model_param_bucket_and_shard_main_param_int32_view_map[
                ...
                for model_fp16_params_this_group, shard_fp32_from_float16_group in zip(
                    ...
                    for i, (model_param, shard_fp32_main_param) in enumerate(
                        ...
                        reuse_data_ptr(shard_fp32_from_float16_group[i], model_param_buffer,
...
def fp16_tensor_convert_to_fp32_tensor_deterministic(shard_fp32_from_float16_groups, optimizer):
    ...
    for shard_fp32_from_float16_group in shard_fp32_from_float16_groups:
        for shard_fp32_param in shard_fp32_from_float16_group:
            ...
            for loop in range(loops):
                odd_even_tensor = torch.sign(
                    exp_avg_sq_flatten[loop * TRANSPOSE_BF16_BLOCK_SIZE: (loop + 1) * TRANSPOSE_BF16_BLOCK_SIZE] > 0)
                shard_int32_tensor[loop * TRANSPOSE_BF16_BLOCK_SIZE: (loop + 1) * TRANSPOSE_BF16_BLOCK_SIZE].add_(
                    odd_even_tensor)
            if remain > 0:
...
def fp32_tensor_convert_to_fp16_tensor_deterministic(shard_fp32_from_float16_groups, optimizer):
    ...
    for shard_fp32_from_float16_group in shard_fp32_from_float16_groups:
        for shard_fp32_param in shard_fp32_from_float16_group:
            ...
            for loop in range(loops):
                odd_even_tensor = ((shard_int32_tensor[loop * TRANSPOSE_BF16_BLOCK_SIZE: (
                                                                                                     loop + 1) * TRANSPOSE_BF16_BLOCK_SIZE] & 131071) == 65536).int()
                shard_int32_tensor[loop * TRANSPOSE_BF16_BLOCK_SIZE: (loop + 1) * TRANSPOSE_BF16_BLOCK_SIZE].sub_(
                    odd_even_tensor)
                sign_tensor = torch.sign(odd_even_tensor - 0.5)
                exp_avg_sq_flatten[loop * TRANSPOSE_BF16_BLOCK_SIZE: (loop + 1) * TRANSPOSE_BF16_BLOCK_SIZE].mul_(
                    sign_tensor)
            if remain > 0:
