...
def get_batch(data_iterator, vp_stage: Optional[int] = None, microbatch_id=None):
    ...
    if not args.enable_vocab_parallel and not is_first_or_last_pipeline_stage(vp_stage) and not mtp_on_this_rank and not is_sft:
    ...
    if not args.enable_vocab_parallel and not is_first_or_last_pipeline_stage(vp_stage) and not mtp_on_this_rank:
        assert is_sft
    ...
    batch_data = [batch[key] for key in BATCH_KEYS]
    if args.enable_vocab_parallel and (get_virtual_vocab_parallel_chunk() == embedding_model_chunk_id):
        InputStore.save_batch(microbatch_id, tuple(batch_data))
...
def forward_step(data_iterator, model: GPTModel, return_schedule_plan: bool = False, microbatch_id = None):
    ...
    with stimer(bdata=True):
        ...
        (
            ...
            position_ids,
            tokens,
        ) = get_batch(data_iterator, vp_stage, microbatch_id=microbatch_id)
