...
def moe_permute_index_map_forward(
    ...
    assert num_out_tokens >= 0, f"moe_permute (index map) requires num_out_tokens >= 0, got {num_out_tokens}."
...
def moe_permute_index_map_backward(
    ...
    act_grad = tex.moe_permute_bwd(grad_permuted_act, dtype, row_id_map, torch.empty(0), num_tokens, topK)
    return act_grad
...
def _moe_permute_index_map_backward_wrapper(
    ...
    act_grad = torch.ops.te_moe.permute_index_map_bwd(grad_permuted_act, row_id_map, ctx.num_tokens, ctx.topK)
...
def moe_unpermute_index_map_backward(
    ...
    act_grad, prob_grad = tex.moe_unpermute_bwd(unpermuted_act_grad, fwd_input, dtype, row_id_map, probs)
...
def moe_permute_mask_map_forward(
    ...
    if not routing_map.is_cuda:
        raise ValueError(f"routing_map must be a CUDA tensor, but got tensor on {routing_map.device}.")
    ...
    if pad_offsets is not None:
        if not pad_offsets.is_cuda:
            raise ValueError(f"pad_offsets must be a CUDA tensor, but got tensor on {pad_offsets.device}.")
...
def _moe_permute_mask_map_backward_wrapper(
    ...
    assert not isinstance(grad_output, QuantizedTensor), "The backward of moe_permute does not support FP8."
...
def moe_permute_and_pad_with_probs(
    ...
    if tokens_per_expert is None:
        raise ValueError("tokens_per_expert must be provided to the fused permute padding function.")
    if align_size <= 0:
    ...
    if torch.equal(tokens_per_expert, target_tokens_per_expert):
        ...
        pad_offsets = torch.cat([torch.zeros(1, dtype=cum_pad.dtype, device=inp.device), cum_pad[:-1]])
