...
def broadcast_obj(obj):
    ...
    obj_shape_tensor = torch.tensor(obj_tensor.shape, device=torch.cuda.current_device())
    broadcast_in_mp_dp(obj_shape_tensor, src, mp, dp)
    obj_len = obj_shape_tensor.cpu().tolist()
    if global_rank == src:
        obj_tensor_hcu = obj_tensor.cuda()
    else:
        obj_tensor_hcu = torch.empty(obj_len, dtype=torch.uint8, device=torch.cuda.current_device())
    broadcast_in_mp_dp(obj_tensor_hcu, src, mp, dp)
    result = pickle.loads(obj_tensor_hcu.cpu().numpy().tobytes())
    del obj_tensor_hcu
    return result
