...
class SwapManager(metaclass=SingletonBase):
    def __init__(self):
        ...
        self.oom_rescue_stream = torch.cuda.current_stream()
    ...
    def prefetch_pack(self, origin_tensor):
        ...
        if swap_tensor.is_slice_tensor:
            ...
            swap_tensor.h2d_event = torch.cuda.Event()
        else:
            if swap_tensor.storage_data_ptr not in self.cpu_tensors:
                ...
                self.cpu_tensors_h2d_events[swap_tensor.storage_data_ptr] = torch.cuda.Event()
        ...
        if AdaptiveStepMgr().is_swap_profiling_step():
            start_pack_event = torch.cuda.Event(enable_timing=True)
