...
def run_transformer_layer_a2a_overlap_with_capture(model, input_tensors, microbatches, overlap_ep_comm_with_split_attn=False):
    """
    ...
    Returns:
        dict: A dictionary containing model outputs and parameter gradients.
    """
    if overlap_ep_comm_with_split_attn:
    ...
    output, _ = layer_schedule_plan_cls.run(
    ...
    _, b_grad = layer_schedule_plan_cls.run(None, layers[-1], f_input=None, b_grad=torch.ones_like(output))
    grads.append(b_grad)
    torch.cuda.synchronize()
    capture = {
...
def run_mtp_layer_a2a_overlap_with_capture(
    ...
    output_tensors = []
    # forward for 1st microbatch
    f_input, _ = layer_schedule_plan_cls.run(
...
class TestA2AOverlap:
    ...
    @pytest.mark.skipif(not is_te_min_version("1.9.0.dev0"), reason="Requires TE >= 1.9.0.dev0")
    @pytest.mark.parametrize("overlap_ep_comm_with_split_attn", [False, True])
    def test_transformer_layer_overlap_shared_expert(self, overlap_ep_comm_with_split_attn):
        """
    ...
    @pytest.mark.parametrize("fp8_flag", [None])  # fp8 is not supported for bmz
    @pytest.mark.parametrize("overlap_ep_comm_with_split_attn", [False, True])
    def test_mtp_layer_overlap(self, dispatcher_type, fp8_flag, overlap_ep_comm_with_split_attn):
        """
