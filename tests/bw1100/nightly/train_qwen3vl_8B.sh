#!/bin/bash
# CI-maintained copy of examples/qwen3/train_qwen3vl_8B.sh. Kept separate so
# nightly can pass train_iters via env without touching the shared example.
...
TRAIN_ITERS=${train_iters:-${TRAIN_ITERS:-50}}
...
# train iteration hyperparameters (TRAIN_ITERS already resolved above)
LR_WARMUP_ITERS=1
